...
def generate_combinations(version):
    tuple_list = [i[1] for i in alignment_placements if i[0] == int(version)]
    all_combinations = []
    ...
    for tuples in tuple_list:
        for num1 in tuples:
            if num1 != 0:
                for num2 in tuples:
                    if num2 != 0:
                        all_combinations.append(((num1, num2), 0))
    sorted_list = sorted(all_combinations, key=lambda x: (x[0][0], x[0][1]))
    return sorted_list

def get_alignment_placements(version):
    alignment_placement = generate_combinations(version)
    alignment_shapes = []
    for coords, state in alignment_placement:
        pattern_shape = make_shape()
        x = coords[0]
        y = coords[1]
        alignment_shapes.append(((x, y), pattern_shape))
    return alignment_shapes
